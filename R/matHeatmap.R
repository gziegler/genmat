  ...
  function( mats , matnames=NULL , sorton=1 , sort.methods="none" , gene.list = NULL , flip = FALSE , flip.range = c("-50,0","0,50") , flip.method = "max" , align.range = c(-200,0) , align.method = "max" , align = FALSE , sort.ranges = NA , sort.breaks = NA , numgroups=3 , crossmat=NULL , matcolors=NULL , groupcolors=NULL , heatmap.lims="5%,95%" , heatmap.colors="black,white" , agg.lims= NA , fragmats=NULL , fragmatnames=NULL , fragrange=c(0,200) , vplot.colors="black,blue,yellow,red" , vplot.lims="0,95%" , forcescore=TRUE , threads=getOption("threads",1L) ){
    ...
    if(is.null(fragmats) == FALSE){
      ...
      for(v in 1:numfragmats){
        ...
        # draw heatmap of all
        vplotname<-paste(pwd,"/",dname,"/","vplot_all_",fragmatnames[v],".pdf",sep="")
        pdf(width=12,height=12,file=vplotname)
        ...
        if(!missing(mats)){
          ...
          for(q in 1:numgroups[1]){
            ...
            vplotname<-paste(pwd,"/",dname,"/","vplot_group",q,"_",fragmatnames[v],"_pergene.pdf",sep="")
            ...
            #This originally had it opening png, printing heatmap, closing png, opening same png, printing and closing. I think this would then only keep the second heatmap
            #I changed it so it should just put one per page in the pdf. -Greg
            pdf(width=12,height=12,file=vplotname)
    ...
    #process each matrix
    hmnames<-paste(pwd,"/",dname,"/","heatmap_",basename(matnames),".pdf",sep="")
    ...
    for(l in 1:nummats){
      ...
      pdf(file=hmnames[l],height=20,width=20)
